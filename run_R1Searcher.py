...
def parse_args():
    ...
    parser.add_argument("--gpu_id", type=str, default="4,5,6,7") # 默认改为0，方便测试
    # 修改为你的实际路径
    parser.add_argument("--model_path", type=str, default="/disk/Yitong/models/XXsongLALA/Qwen-2.5-7B-base-RAG-RL")
    ...
    parser.add_argument("--output_file", type=str, default="r1_results.csv")
...
# --- R1Searcher 原生 Prompt 处理 ---
def process_text(sample,type=None):
...
# --- R1Searcher 原生 Search ---
# --- R1Searcher 原生 Search (修正版) ---
    ...
    try:
        ...
        def _passages2string(retrieval_result):
            ...
            for idx, doc_item in enumerate(retrieval_result):
                ...
                # 【修正】恢复 Search R1 原生的 Title 拆分逻辑
                parts = content.split("\n")
                if len(parts) > 1:
                    title = parts[0]
                    text = "\n".join(parts[1:])
...
def extract_think_and_answer(output_text):
    ...
    query_matches = re.findall(r'<\|begin_of_query\|>(.*?)<\|end_of_query\|>', output_text, re.DOTALL)
    query_content = [match.strip() for match in query_matches] if query_matches else []
    
    result = {
        "all_think_contents": think_content,
        "all_answer_contents": answer_content,
        "all_query_contents": query_content,
        "last_think": think_content[-1] if think_content else None,
        "last_answer": answer_content[-1] if answer_content else None,
        "last_query": query_content[-1] if query_content else None,
    }
    return result

# ==========================================
# 核心处理逻辑 (集成 RIND + R1 Loop)
# ==========================================
def process_single_question_r1(question, model, tokenizer, rind_calculator, device, filter_judge, prompt_type="v0"):
    # 1. 初始化 Prompt
    sample = {"question": question}
    processed_sample = process_text(copy.deepcopy(sample), type=prompt_type)
    if not processed_sample: return None
    
    current_prompt = processed_sample["chat_prompt"]
    ...
    # 3. 状态管理
    full_process_log = current_prompt # 记录全过程
    current_search_context = "Internal Knowledge (No Search)" # 当前轮次生成的上下文
    
    max_rounds = 5
    current_round = 0
    
    while current_round < max_rounds:
        # print(f"DEBUG: Round {current_round+1}/{max_rounds}")
        
        # 准备输入 (注意：R1Searcher 这里的 prompt 是纯文本堆叠，不使用 apply_chat_template)
        ...
        # === 生成 (带 Score & Attention) ===
        # 使用 RIND 需要的详细输出模式
        outputs = model.generate(
            input_ids, 
            max_new_tokens=400, # R1 default
            do_sample=True, # R1 default usually False or True depending on config, here use True for variety
            temperature=0.7,
        ...
        gen_text_chunk = tokenizer.decode(gen_seq, skip_special_tokens=True) # 用于逻辑判断
        # print(f"Output chunk: {gen_text_chunk[:50]}...")
        
        # === RIND 计算与分句 (针对本轮生成的 chunk) ===
        # 只有当生成了内容才计算
        if len(gen_seq) > 0:
            class MiniOut:
            ...
            # 计算 RIND
            full_input_ids = outputs.sequences # 包含 input + output
            try:
                ...
                for span in doc.sents:
                    ...
                    collected_contexts.append(current_search_context) # 使用当前轮次的上下文
                    
            except Exception as e:
                logger.error(f"RIND calculation error in round {current_round}: {e}")
                # Fallback: 存整个 chunk，分数为 0
        ...
        # === R1Searcher 流程控制 ===
        output_extract = extract_think_and_answer(gen_text_chunk) # 注意：这里解析的是本轮 chunk
        
        # 1. 检查是否有了最终答案
        if output_extract["last_answer"]:
            # 结束
            break
            
        # 2. 检查是否发起查询
        elif output_extract["last_query"]:
            query = output_extract["last_query"]
            # 清洗 query
            query = query.replace('"', "").replace("'", "").replace("\t", " ").replace("...", "").strip()
            if query:
                # print(f"Search triggered: {query}")
                search_res_str = search(query)
                
                if search_res_str:
                    # 构造注入内容
                    injection = f"\n<|begin_of_documents|>\n{search_res_str}<|end_of_documents|>\n\n"
                    # 更新 Prompt 供下一轮使用
                    current_prompt += gen_text_chunk # 先加上本轮生成的
                    current_prompt += f"\n\n<|begin_of_documents|>\n{search_res_str}<|end_of_documents|>\n\n"
                    
                    # 更新 Log
                    # full_process_log += injection
                    
                    # 【关键】更新 Context 变量，供【下一轮】生成的句子使用
                    # 清洗 search result 用于 CSV 记录
                    clean_res = search_res_str.replace("\n", " ").strip()
                    current_search_context = f"[Query: {query}] {clean_res}"
                else:
                    # 检索为空，可能出错或无结果
                    # 终止或继续？R1 代码选择 break，我们这里 break
                    break
            else:
                # Query 解析失败
                break
        
        # 3. 既无 Answer 也无 Query -> 可能是中间推理过程，或者模型停了
        else:
            # 如果模型没生成 Query 也没生成 Answer 只是生成了一段话然后停了(EOS)
            # R1 逻辑是 "shot_down"，这里我们也 break。
            # 如果是因为 max_new_tokens 截断，那可能会有问题，但 R1 设定了 400 应该够输出一个动作
            # 简单处理：更新 prompt 继续（如果没结束符）？
            # 你的 R1 代码逻辑是 break，我们保持一致。
            break
...
def main():
    ...
    # 加载数据
    dataset = []
    if "truthfulqa" in args.dataset: # 注意这里用了 dataset 参数
        print("Loading TruthfulQA...")
        ds = load_dataset("truthfulqa/truthful_qa", "generation")
        for item in ds['validation']:
            dataset.append(item['question'])
    elif "halueval" in args.dataset: # 注意这里用了 dataset 参数
        # 兼容你的 src_file 逻辑，这里统一用 dataset 判断
    ...
    model_base_name = os.path.basename(args.model_path.rstrip('/'))
    args.output_file = f"{model_base_name}_{args.dataset}_searchr1_results.csv"
    ...
    for i, question in enumerate(tqdm(dataset)):
        try:
            # 执行 R1 + RIND 流程
            full_ctx, raw_scores, raw_sents, raw_ctxs, filt_scores, filt_sents = process_single_question_r1(
                question, model, tokenizer, rind_calculator, 
                device, filter_judge, prompt_type=args.prompt_type
            )
